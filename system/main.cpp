...
inline float GetCpuUsageRatio(int pid)
{
    ...
    int64_t time_delta = time - last_time;
    if (time_delta == 0)
    {
    ...
    int64_t system_time_delta = system_time - last_system_time;
...
int main()
{
    ...
    for (int i = 0; i < 5000; i++)
        thread([&i, &j]
               { this_thread::sleep_for(chrono::milliseconds(10)); j = i*i*i*i*i; })
            .detach();
...
// g++ main.cpp -lpsapi -o main && main.exe
